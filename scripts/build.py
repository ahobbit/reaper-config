...
for platform in (['windows','macos'] if args.platform=='all' else [args.platform]):
    ...
    shutil.copytree(ROOT/'common',dest/'Configuration')
    for item in (ROOT/'platforms'/platform).iterdir():
        if item.is_dir():shutil.copytree(item,dest/'Configuration'/item.name,dirs_exist_ok=True)
        elif item.suffix=='.ini':shutil.copy2(item,dest/'Configuration'/item.name)
        else:shutil.copy2(item,dest/item.name)
    shutil.copytree(ROOT/'fonts',dest/'Fonts')
    (dest/'Installers').mkdir()
    for dep in dependencies:shutil.copy2(ROOT/'downloads'/dep['name'],dest/'Installers'/dep['name'])
